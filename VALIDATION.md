...
(very little — realized ς below ≈0.1) and is the reason a dispersion gate precedes any
...
| FDR does not blow up in real null regimes | **NOT validated** | Only synthetic nulls measured (above). ADR-0050 now bounds the *synthetic* tolerance — selection validity dies at realized per-leaf scale dispersion ς ≈ 0.1, well inside plausible real-fabric heterogeneity — but real-fabric ς itself is unmeasured. The recorded prerequisite for any real-fabric FDR claim is a ς̂ dispersion gate (ADR-0050 follow-up), not more synthetic evidence. |
