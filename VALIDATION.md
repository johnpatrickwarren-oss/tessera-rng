...
| ς̂ dispersion gate (ADR-0051) | `coverage-matrices/dispersion-gate.{json,md}`: recovery + operating characteristic on the known-ς generator + a tail-contamination kill-test, n=8/cell | The PAIR gate — max(robust ς̂, tail ς̂); the robust statistic alone launders tail-contaminated fleets (cold-eye-demonstrated, corrected on the record) — passes 100% on clean fabrics (ς̂ 0.009/0.006) and **fails 100% at every cell where selection lies**, including the 10%-at-2× contaminated fleet the robust core is blind to; the threshold-straddling cell (realized ς 0.059 vs ς\*=0.05) passes 13% — conservative by design. Tail ς̂ tracks realized ς almost exactly (0.335 vs 0.353). Gates the FDR **claim**, never the alarm. ROC measured on the Gaussian-ς family + the two-point contamination case at 109 leaves. **SCALE LIMIT CLOSED (ADR-0061): the gate now binds on a TRIPLE** — the pair AND the extreme-leaf z_max against a Bonferroni bound Φ⁻¹(1−0.01/n), N-indexed from theory. The pair-era laundering (ADR-0059: pass 100% while e-BH false-selects at (1456–6112, ς=0.05)) is measured CLOSED — those cells now fail 100% via z_max; laundering 0 everywhere; clean-at-scale cells still pass 100%; near-threshold straddle cells turned fully conservative (13% → 0% pass — claims withheld, none laundered). |
| Onset vs N (ADR-0059) | `coverage-matrices/onset-scale.{json,md}`: ς grid × {109, 1456, 6112} leaves × {shared, perLeafScale}, gate + LAUNDERING columns per cell; the 109/shared ς∈{0.05, 0.1} cells anchored to ADR-0050; n=8/5/3 by size | **The GPU sibling's N13 transfers: the dispersion onset collapses with N (0.075 → ≤0.05, a one-sided bound), into the fixed gate threshold — a measured LAUNDERING region (gate passes 100%, e-BH false-selects 2/5 and 2/3 of runs) at paper scale.** Mild in magnitude at tested sizes (≤0.67 false sel/run), confined to the near-threshold band; measured points monotone in N (not resolved at these n). **`perLeafScale` is N-robust: 0 false selections in all 15 remedy cells across a 56× size span** — the RNG mirror of the GPU rack-local fix. The pair-era laundering is HISTORICAL: the operator took the parked decision ((a)+(b)) and the artifact now records the ADR-0061 triple gate — laundering 0 in every cell, former laundering cells failing 100% via z_max. |
| FDR license rule (ADR-0060) | `src/license.ts` + `test/license.test.ts`: truth table + e2e both directions + batch/session parity | **The license is code: an audit's selection set may be read as FDR-controlled iff `calibration_construction: 'per_leaf_scale'` AND `drift_monitor.status: 'ok'`.** Shared-calibration audits are NEVER licensed (ADR-0059: the fixed gate launders at scale under shared calibration; ADR-0057: real fleets sit far past any fixed threshold) — they are Mode-A evidence/ranking. An unmonitored or unresolvable window refuses. Every refusal names its conjunct. |
| Anytime alarms (ADR-0062) | `coverage-matrices/eop-alarms.{json,md}`: clean pooled ever-alarm fraction vs α + faulted first-alarm latency, n=8; session-only | **The Ville rule at 1/α on the tick-valid family mean (A+C)/2 — P(a null leaf EVER alarms) ≤ α UNDER THE CALIBRATED NULL, no patience parameter** (a genuine supermartingale; Family D excluded, ADR-0044). **The conditionality is load-bearing: dispersion/drift voids the guarantee exactly as it voids e-BH validity — the alarm read carries the same gate/monitor (or perLeafScale) preconditions as the evidence surface.** Measured: pooled clean ever-alarm fraction 1.95% (17/872) ≤ α=5% (the guaranteed quantity; per-seed counts are binomial reference data); the fleet (α/n) scope silent 8/8; faulted leaves alarm 16/16, mostly within single-digit ticks. Alarms are DETECTION — ADR-0060 untouched; stopped e-BH remains unlicensed (ADR-0043). |
| Escalation tier (ADR-0064) | `coverage-matrices/resource-eprocess.{json,md}`: clean validity + the ADR-0049 probe table under the honest null + the fault-side sibling comparison, n=4/cell, 76 resources | **Resource-aggregate e-processes with CALIBRATED nulls (0 clean escalations in 4×76) catch a room fault at HALF the leaf-layer floor (Δ=0.5: 4/4 vs 0/4); the probe table REPRODUCES (panel Δ=1: 3/4; optic: within seed noise)** — an earlier "narrow wins are null-error artifacts" claim is RETRACTED (it was a typo'd-resource silent no-op measuring clean data; the generator now throws — guard-tested). Claim granularity = NEIGHBORHOODS (hood sizes published; **rooms = the fleet on this fabric**, escalation order ≠ localization — both disclosed). The sibling confound is real and STRONGER under the honest null (calibrated room-1 E ≈ 1e5+ — the fault-side mutant-killer pin). Conditional on the calibrated null (gate/monitor preconditions apply). |
...
attribution) and that the remedy restores it. The license rule across the set (ADR-0060,
superseding the earlier gate-AND-monitor phrasing): FDR-bearing readings require the
per-leaf construction AND monitor-ok (`src/license.ts`); everything else is Mode-A
evidence/ranking, and the (triple) gate is the Mode-A honesty instrument.
