...
and the per-leaf correction (ADR-0052) removes its static part — at the measured cost of a
drift cliff that no shipped component detects (the gate cannot see staleness); only a
fresh-calibration cadence guards it until a runtime drift monitor exists.
